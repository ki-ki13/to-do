...
export default function MemeGenerator({ goal }: MemeGeneratorProps) {
  ...
  const generateMeme = async () => {
    ...
    try {
      ...
      const result = await AIMemeGenerator.generateVariedMeme(completedTasks, totalTasks, goal);
  ...
  const downloadMeme = () => {
    if (memeResult?.imageUrl) {
      ...
      link.download = `daily-meme-${new Date().toISOString().split("T")[0]}.png`;
          ...
          <div>
            ...
            <div className="space-y-3">
              <div className="text-sm text-gray-600 bg-gray-50 p-3 rounded-lg">
                <p><strong>Method:</strong> {memeResult.method}</p>
                <p><strong>Template:</strong> {memeResult.memeData.template}</p>
                <p><strong>Type:</strong> {memeResult.memeData.type}</p>
                {memeResult.memeData.isAIGenerated && (
                  <p><strong>AI Generated:</strong> ✨ Yes</p>
                )}
              </div>
        ...
        {!process.env.NEXT_PUBLIC_GROQ_API_KEY && (
          <div className="mt-4 p-4 bg-yellow-50 border-l-4 border-yellow-400 text-yellow-700 rounded">
            <div className="flex">
              ...
              <div className="ml-3">
                <p className="text-sm font-medium">Setup Required for AI Features</p>
                <p className="text-sm mt-1">
                  Add <code className="bg-yellow-100 px-1 rounded">NEXT_PUBLIC_GROQ_API_KEY</code> to your environment variables for AI-generated text.
                </p>
                <p className="text-xs mt-2">
                  Get your free API key at <a href="https://console.groq.com/" target="_blank" rel="noopener noreferrer" className="underline hover:text-yellow-800">console.groq.com</a>
                </p>
              </div>
            </div>
          </div>
